...
methods = methods[ -grep( c("MM"), methods) ]
methods = methods[ -grep( c("permscdd"), methods) ]
methods

match = match( methods,  all_methods )

sel_distinct = grep("distinct", methods)
methods[sel_distinct] = paste0(methods[sel_distinct], "3")
...
gg_fdr[[1]] +   theme(legend.position="none")
gg_fdr[[2]] +   theme(legend.position="none")
gg_fdr[[3]] +   theme(legend.position="none")
gg_fdr[[4]] +   theme(legend.position="none")
gg_fdr[[5]] +   theme(legend.position="none")
...
ggsave(filename = paste0("FDR-by-type-", pairing, "-SCnorm.pdf"),
       plot = AA,
       device = "pdf",
