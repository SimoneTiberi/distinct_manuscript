rm(list = ls())
setwd("~/Desktop/distinct project/SIMULATED data/EXTRA SIMULATIONS/n significant genes/")
...
for(n_cells in CELLS){
  ...
  if(n_cells == 10){ # original simulation location"
    ...
    for(i in seq_along(types)){
      ...
      for(rep in 1:n_rep){
        if(n_cells == 10){
          ...
          sel_type = grep(paste0(types[i], "200", "_significant_", sub("\\.", "", n_cells), ",", rep), res_names[sel_method])
        }
        
        x = readRDS(res_names[sel_method][sel_type])$tbl
        
        #<simpleError in assay(object, i = exprs_values, ...): 'assay(<SingleCellExperiment>, i="character", ...)' invalid subscript 'i'
        #'normcounts' not in names(assays(<SingleCellExperiment>))>
...
AA = egg::ggarrange( plots = 
                       list(FDR_ALL[[1]] + xlab("") + labs(title = "1% DGE") + theme(legend.position = "none", plot.title = element_text(hjust = 0.5, face = "bold", size=35)),
                            FDR_ALL[[5]] + xlab("") + ylab("") + labs(title = "5% DGE") + theme(legend.position = "none", plot.title = element_text(hjust = 0.5, face = "bold", size=35)),
                            FDR_ALL[[10]] + labs(title = "10% DGE") + theme(legend.position = "none", plot.title = element_text(hjust = 0.5, face = "bold", size=35)),
                            FDR_ALL[[20]] + ylab("") + labs(title = "20% DGE") + theme(legend.position = "none", plot.title = element_text(hjust = 0.5, face = "bold", size=35))),
                     bottom = ggpubr::get_legend(FDR_ALL[[1]]),
                     ncol = 2, nrow = 2)


ggsave(filename = paste0("FDR-overall-n-significant-genes.pdf"),
       plot = AA,
       device = "pdf",
