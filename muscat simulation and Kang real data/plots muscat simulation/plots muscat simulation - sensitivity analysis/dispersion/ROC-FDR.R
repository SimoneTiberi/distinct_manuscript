rm(list = ls())
setwd("~/Desktop/distinct project/SIMULATED data/EXTRA SIMULATIONS/dispersion/")
    ...
    for(i in seq_along(types)){
      ...
      for(rep in 1:n_rep){
        if(n_cells == 1){
          ...
          sel_type = grep(paste0(types[i], "200", "_disp_", sub("\\.", "", n_cells), ",", rep), res_names[sel_method])
        }
...
AA = egg::ggarrange( plots = 
                       list(FDR_ALL[[2]] + xlab("") + labs(title = "1/5 * dispersion") + theme(legend.position = "none", plot.title = element_text(hjust = 0.5, face = "bold", size=35)),
                            FDR_ALL[[5]] + xlab("") + ylab("") + labs(title = "1/2 * dispersion") + theme(legend.position = "none", plot.title = element_text(hjust = 0.5, face = "bold", size=35)),
                            FDR_ALL[[20]] + labs(title = "2 * dispersion") + theme(legend.position = "none", plot.title = element_text(hjust = 0.5, face = "bold", size=35)),
                            FDR_ALL[[50]] + ylab("") + labs(title = "5 * dispersion") + theme(legend.position = "none", plot.title = element_text(hjust = 0.5, face = "bold", size=35))),
                     bottom = ggpubr::get_legend(FDR_ALL[[50]]),
                     ncol = 2, nrow = 2)

ggsave(filename = paste0("FDR-overall-dispersion.pdf"),
       plot = AA,
       device = "pdf",
