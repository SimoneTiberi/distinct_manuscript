rm(list = ls())
setwd("~/Desktop/distinct project/SIMULATED data/EXTRA SIMULATIONS/library size/")
...
# NEW RESULTS:
res_names_new = list.files(paste0("results"), full.names =TRUE)
file.exists(res_names_new)

# original results
res_names_original = list.files("~/Desktop/distinct project/SIMULATED data/FULL muscat pipeline/results/results_7Samples/", full.names =TRUE)
file.exists(res_names_original)
...
min_sim_mean = -1 # filter below (min 20 non-zero cells)
...
min_20_cells_names = list.files(paste0("min_50_cells"), full.names =TRUE)
file.exists(min_20_cells_names)

FILTER = list()
for(n_cells in CELLS){
  FILTER[[n_cells * 10]] = list()
  for(i in seq_along(types)){
    FILTER[[n_cells * 10]][[i]] = list()
    for(rep in 1:n_rep){
      
      if(n_cells == 1){
        sel_filter = grep(paste0(types[i], "200,", rep), min_20_cells_names)
      }else{
        sel_filter = grep(paste0(types[i], "200", "_lib_", sub("\\.", "", n_cells), ",", rep), min_20_cells_names)
      }
      
      # sel filter (min 20 non-zero cells):
      FILTER[[n_cells * 10]][[i]][[rep]] = readRDS(min_20_cells_names[sel_filter])
    }
  }
  print(n_cells)
}
...
for(n_cells in CELLS){
  ...
  if(n_cells == 1){ # original simulation location"
    res_names = res_names_original
  }else{ # new additional simulations:
    res_names = res_names_new
  }
    ...
    for(i in seq_along(types)){
      ...
      for(rep in 1:n_rep){
        if(n_cells == 1){
          ...
          sel_type = grep(paste0(types[i], "200", "_lib_", sub("\\.", "", n_cells), ",", rep), res_names[sel_method])
        }
        
        x = readRDS(res_names[sel_method][sel_type])$tbl
        
        # sel filter (min 20 non-zero cells):
        cluster_gene = FILTER[[n_cells * 10]][[i]][[rep]]
        
        x$cluster_gene = paste(x$cluster_id, x$gene, sep = "-")
        sel = x$cluster_gene %in% cluster_gene
        x = x[sel,]
  ...
  status = data.frame(status = res[[1]]$is_de[sel_genes])
  
  rownames(p_val) = rownames(p_adj) = rownames(status) = 1:nrow(p_val)
  ...
  #ROC_ALL[[n_cells * 10]] = plot_roc(cobraplot)
  
  FDR_ALL[[n_cells * 10]] = plot_fdrtprcurve(cobraplot, plottype = c("points"),
...
AA = egg::ggarrange( plots = 
                       list(FDR_ALL[[2]] + xlab("") + labs(title = "1/5 * library size") + theme(legend.position = "none", plot.title = element_text(hjust = 0.5, face = "bold", size=35)),
                            FDR_ALL[[5]] + xlab("") + ylab("") + labs(title = "1/2 * library size") + theme(legend.position = "none", plot.title = element_text(hjust = 0.5, face = "bold", size=35)),
                            FDR_ALL[[20]] + labs(title = "2 * library size") + theme(legend.position = "none", plot.title = element_text(hjust = 0.5, face = "bold", size=35)),
                            FDR_ALL[[50]] + ylab("") + labs(title = "5 * library size") + theme(legend.position = "none", plot.title = element_text(hjust = 0.5, face = "bold", size=35))),
                     bottom = ggpubr::get_legend(FDR_ALL[[50]]),
                     ncol = 2, nrow = 2)

ggsave(filename = paste0("FDR-overall-lib-size.pdf"),
       plot = AA,
       device = "pdf",
